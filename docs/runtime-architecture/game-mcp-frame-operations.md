...
| Requirement evaluation: selected current value, required scaled/effective threshold, met/unmet, availability/visibility/discovery/mastery/quantity/level inputs | 250-ms dynamic | Structural graph is re-derived against current published entity rows. |
| Native `Check(ConditionInfo)` owner/level/verdict for the explained entity | request-time | `WorldRequirementNativeVerdictProbe` asks the game for that one entity inside the frame operation, so the answer is this frame's. Only the parameterised overload is bound; it neither stamps nor latches. Native/suite disagreement fails loud. |
...
| Detail-read predicates: visible, available, canDevelop, canPurchase, canDiscover, canUse; stable false reason codes; queue/cap/leeway/discovery/bandwidth/drain blockers | 250-ms dynamic | Pure typed evaluation over one pinned world, plus the structural tables retained in that world. One `world_get` call evaluates once per id it was handed. No snapshot token or generation crosses calls. |
...
| Tooltip catalog paths, and the plain screen text of the one container the screen drew | Request-time | `game_screen_elements`/`game_tooltip` only, through lifecycle-bound tooltip accessors on Unity. Paths are explicitly volatile screen-state handles. The catalog is scoped to elements the player can point at: a closed `UIModal` keeps its subtree active in the hierarchy, so the walk asks the game's own `IsOpen` up each element's ancestry rather than trusting Unity liveness. It merges the hoverable elements with the chrome controls the suite offers — on screen, and in the named player-chrome set — which draw no tooltip and are the only rows carrying `opens` rather than an id. The text read is the core panel, its non-recursive sub-panel row, and one pointer line per linked tooltipable; computed delegates run inline and a link is never followed. Rich-text and node-tree ceremony are removed after the one requested read; unrelated operations make zero tooltip calls. |
