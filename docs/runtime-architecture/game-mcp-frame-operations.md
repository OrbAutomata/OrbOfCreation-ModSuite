...
| Tooltip catalog paths and plain screen text, including nested/computed/inspected panels | Request-time | `game_screen_elements`/`game_tooltip` only, through lifecycle-bound tooltip accessors on Unity. Paths are explicitly volatile screen-state handles. The catalog is scoped to elements the player can point at: a closed `UIModal` keeps its subtree active in the hierarchy, so the walk asks the game's own `IsOpen` up each element's ancestry rather than trusting Unity liveness. It merges the hoverable elements with the chrome's `UIModalActivator` controls, which draw no tooltip and are the only rows carrying `opens` rather than an id. Rich-text and node-tree ceremony are removed after the one requested read; unrelated operations make zero tooltip calls. |
