...
internal sealed class GameMcpProtocolRouter
{
    ...
    private static JObject ListTools() => new()
    {
        ["tools"] = new JArray
        {
            ...
            Tool(
                "entity_catalog",
                "Search what this build loaded that the world publishes no row for",
                "Search the words the game shows a player that the published world carries no row for: the ritual and combat glossary this server names nowhere else (statuses, damage types, combatant stats, enemy prefixes, combat actions, enchantments, glyph types, rune stones, creature types, and the three display types every statistic is tagged with), the stat groups behind the Statistics tab, and this build's authored oddities. It is a short page on purpose. Everything the world does publish is found with world_search and paged with world_list, which carry the price, the state, the requirements and the description this page could never hold — an entity is on one side or the other, never both, so there is nothing here to cross-check against them. A query is matched case-insensitively as a substring against the entity's id, its native type, its internal asset name and its player-facing display name, so a loaded asset progression has not revealed yet is findable without navigation. The build's internal machinery is not listed either: localization bundles, scaling curves, animations and colours, the one-slot variables a screen keeps its cursor and selection in, RNG salts, key bindings, music tracks, the named list variables whose contents are already a published category, and the prerequisite-link nodes world_get already expands tier by tier. No id leaves this build by being off this page: an id handle still resolves against every loaded id, a row that references one still prints its name, and world_get still answers for it by saying what it is and that its identity is all there is to read. A row carries uuid and nativeType, name where the game authors a word for it, and internalName where the Unity asset id is not that name with its spaces and punctuation taken out. Rows page in UUID order, which is not world_search's relevance order. limit is an upper bound: a page also stops at a 12 KB response budget, so wide rows come back short. nextOffset is present exactly when more rows remain, and is the offset to resume from.",
                ObjectSchema(
                    new JObject
                    {
                ...
                ObjectSchema(
                    new JObject
                    {
                        ...
                        ["keyword"] = StringSchema(
                            "The id of a type asset, as a row prints it. Narrows to the things " +
                            "that wear it — the far side of its members count."),
                        ["offset"] = IntegerSchema(0, int.MaxValue),
                        ["limit"] = IntegerSchema(1, 200),
                    })),
            Tool(
