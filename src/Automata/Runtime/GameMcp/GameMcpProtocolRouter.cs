    ...
    private static JObject ListTools() => new()
    {
        ["tools"] = new JArray
        {
            ...
            Tool(
                ...
                "Search the assets this build loaded that a reader could act on knowing: every entity the published world carries a row for, plus the loaded ones it carries no row for that are still words the game shows a player — the ritual and combat glossary this server names nowhere else (statuses, damage types, combatant stats, enemy prefixes, combat actions, enchantments, glyph types, rune stones, creature types, and the three display types every statistic is tagged with), the stat groups behind the Statistics tab, and this build's authored oddities. A query is matched case-insensitively as a substring against the entity's id, its native type, its internal asset name and its player-facing display name, so a loaded asset progression has not revealed yet is findable without navigation. The game's internal machinery is deliberately not listed: localization bundles, scaling curves, animations and colours, the one-slot variables a screen keeps its cursor and selection in, RNG salts, key bindings, music tracks, the named list variables whose contents are already a published category, and the prerequisite-link nodes world_get already expands tier by tier. Those ids have not moved: an id handle still resolves against every loaded id, a row that references one still prints its name, and world_get still answers for it by saying what it is and that its identity is all there is to read. category reads not-world-projected for a listed asset the published world has no row of its own for. Rows page in UUID order, which is not world_search's category order. limit is an upper bound: a page also stops at a 12 KB response budget, so wide rows come back short. nextOffset is present exactly when more rows remain, and is the offset to resume from.",
                ObjectSchema(
                    new JObject
                    {
